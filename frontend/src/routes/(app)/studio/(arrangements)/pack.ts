...
function packSquares(values: number[], maxValue: number) {
    ...
    const squares = <Square[]>[
        { x: 0, y: 0, width: square0Size, height: square0Size },
    ];
    ...
    values.slice(1).forEach((value) => {
        ...
        const v0Index = outline.reduce(
            (a, v, i) =>
                distance$1(v, centroid) < distance$1(outline[a], centroid)
                    ? i
                    : a,
            0,
        );
        ...
        const toInsert = [
            distance$1(v2, n2) < eps ? undefined : v2,
            v4,
...
// ----------------------------------------------------------------------------
// Vec2
const distance$1 = ([x1, y1]: number[], [x2, y2]: number[]) =>
...
function genSquares(min_size: number, max_size: number, n_squares: number) {
    ...
    var outline = result.outline.concat([
        result.outline[0],
        result.outline[1],
        result.outline[2],
    ]);
...
function mirror(squares: Square[], axis: number, sample_point: number[]) {
    for (var i = 0; i < squares.length; i++) {
        ...
        if (axis == 0) {
            //If we are rotating around the x axis, change y-values
            var new_y = (squares[i].y - origin) * -1 - squares[i].height;
            squares[i].y = new_y;
        } else {
            //If we are rotating around the y axis, change x-values
...
function pointIn(sqr: PointPair, point: Point) {
    ...
    const between = (low: number, mid: number, top: number) =>
        low < mid && mid < top;

    // If both x and y values of the point are within the x and y bounds of the sqr, there is overlap
    var overlap =
        between(sqr.left.x, point.x, sqr.right.x) &&
        between(sqr.left.y, point.y, sqr.right.y);
...
export function generateFullArrangement(
    scale: number,
    min_squares: number,
    max_squares: number,
    offset_min: number,
    ...
    var arr_A = genSquares(
        1,
        25,
        Math.floor(Math.random() * indv_max + indv_min),
    );
    var arr_B = genSquares(
        1,
        20,
        Math.floor(Math.random() * indv_max + indv_min),
    );
    ...
    // Generate offset
    var pos = genPositions(
        arr_A.longestEdge.length * offset_max,
        arr_A.longestEdge.length * offset_min,
    );
    var axis = pos[0];
    var offset = pos[1];
    ...
    if (checkOverlap(full_arr)) {
        return generateFullArrangement(
            scale,
            min_squares,
            max_squares,
            offset_min,
