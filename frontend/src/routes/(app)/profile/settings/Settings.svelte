<script lang="ts">
    ...
    async function resetProfileInformation() {
        ...
        } else if (info.not_authenticated)
            resetProfileStatus = "failed: not authenticated";
    ...
    async function resetListeningHistory() {
        ...
        if (
            !confirm(
                "Are you sure you want to reset your listening history? " +
                    "This action cannot be undone.",
            )
        )
            return;
        ...
        if (data.success) resetListeningStatus = "reset successfully";
            ...
            resetListeningStatus = "no listening data found";
        else if (data.not_authenticated)
            resetListeningStatus = "failed: not authenticated";
        else if (data.server_error)
            resetListeningStatus = "failed: server error";
    ...
    async function setProfileTheme(themeType: string) {
        ...
        // handle errors
            ...
            themeStatus = "failed: not authenticated";
        else if (info.server_error) themeStatus = "failed: server error";
        else if (info.update_unnecessary)
            themeStatus = "theme already set to that value";
...
<div id="settings">
    ...
    <div id="account-actions">
        ...
        <div class="button-status-group">
            <button class="link-button" onclick={resetListeningHistory}
                >reset listening history</button
            >
            <p role="status">{resetListeningStatus}</p>
        </div>
        <div class="button-status-group">
            <button class="link-button" onclick={resetProfileInformation}
